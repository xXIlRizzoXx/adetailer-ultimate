...
- **Names from a preset, pasted parameters or the API** match regardless of case (`Face` selects `face`; an exact match wins). A name the detector does not have is ignored and named in the console; when no name of an include filter matches, every class is inpainted, as with an empty selection.
...
- **Load** — pick a name from the dropdown and hit **📂 Load**. All widgets in the current tab snap to the preset's values, including the class dropdown for the detector model in use.
- **Rename** — pick a preset, type the new name into the textbox, hit **✏️ Rename**. The on-disk file is updated atomically.
- **Delete** — pick a preset, hit **🗑 Delete**. The selection clears.
