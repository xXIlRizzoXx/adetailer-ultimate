...
- 🟡 Every UI label, accordion title, button, info hint, placeholder, dropdown sentinel value and tooltip exposed by the fork-added widgets is translated into 10 locales (`it_IT`, `es_ES`, `fr_FR`, `de_DE`, `zh_CN`, `ja_JP`, `pt_BR`, `ru_RU`, `ko_KR`, `pl_PL`). In this unreleased beta, the reworded Copy settings and Reset tooltips, the reworded "Reset ADetailer settings" help text on the Settings page and the corrected "Apply only on hires.fix" help text stay English until the dictionaries get the new wording. The translation dictionaries themselves **live in the [Language Diffusion](https://github.com/xXIlRizzoXx/sd-webui-language-diffusion) extension** — installing Language Diffusion alongside ADetailer Ultimate is the recommended path, since that extension also drives the top-bar language selector + automatic page reload on locale switch.
...
| Skip img2img                      | Skip img2img. In practice, this works by changing the step count of img2img to 1. The saved image's parameters keep your own steps, sampler and size (in the img2img Batch tab, each file's own), and "Use same sampler" uses your sampler. With "Inpaint only masked" off, the whole init image is inpainted at its own size, so the result keeps that size instead of the width/height sliders'. An API request with one init image per batch image details each output's own init image.  | img2img only                                                                                                                                           |
...
On AUTOMATIC1111 (and likely classic Forge / reForge, which keep its check) the safe-unpickle check stops the UI from reading class names from a `.pt` without a sidecar. Generation is allowed to read them, so a class filter still works; the CLASSES dropdown lists them after the first detection with that model (switch to another detector and back), or straight away with a sidecar (one added while the WebUI is running is read when you switch to another detector and back).
...
A class name the detector does not have (for example `hands` from a preset, pasted parameters or the API, for a model whose class is `hand`) gets no pass of its own and is named in the console, as in single-pass mode; otherwise its pass would detect every class and repaint the regions of the other passes with that name's prompt. When no selected name is known to the detector, every class is still inpainted, as before.
...
A LoRA your tab prompt already names is not added again, whatever its weight: with `<lora:detail:1>` in the main prompt and `detailed face <lora:detail:0.3>` in the tab, the detailer pass uses the LoRA once, at 0.3. Names are compared as the WebUI reads them, so the letter case must match.
