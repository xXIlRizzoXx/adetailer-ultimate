...
- 🟢 Built-in **"📖 ADetailer Guide"** top-level tab (registered via `on_ui_tabs`, appears in the main tab bar next to Settings): a complete plain-language reference for every option, with a **table of contents** and one **collapsible section per topic** (Getting started, Choosing a detector, Detection, Mask preprocessing, Prompts, Inpainting, Per-pass overrides, Tools, Presets, Fixing common problems, Special tokens, What's new). A small non-invasive pointer at the bottom of the panel links to it. Read-only static markdown (`build_guide_blocks` / `_GUIDE_SECTIONS` / `.ad-guide`) — no event listeners, and the guide tab is a separate `gr.Blocks`, so it's index-safe. The `on_ui_tabs` registration is guarded (a failure just yields no tab). Kept in English as reference text (like the technical vocabulary), so it is not a translation key.
