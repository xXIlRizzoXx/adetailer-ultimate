...
| 🟢 | MediaPipe 1.0 — process abort on macOS | MediaPipe 1.0 (Jul 2026) is a ground-up rewrite: the top-level module exposes only `Image` / `ImageFormat` / `tasks`, and 1.0.1 (14 Aug 2026) moved the `tasks` API onto a raw ctypes bridge that **aborts the whole process** on macOS while creating a `FaceDetector` / `FaceLandmarker`. A native abort is not a Python exception, so no `try/except` can catch it — the WebUI simply dies. A fresh install resolved to 1.0.1 because the dependency had no upper bound. | `install.py` and `pyproject.toml` now cap MediaPipe at the tested **0.10.x** line (`>=0.10.13,<1.0`), so new installs land on a known-good build and an install that already picked up 1.0.x is brought back. As a second layer for `--skip-install` setups, `_tasks_api_aborts()` in `adetailer/mediapipe.py` refuses the `tasks` API on **macOS with MediaPipe ≥ 1.0.1 only** and prints how to fix it, degrading to "nothing detected" instead of killing the WebUI. Windows and Linux are untouched (verified: the same call completes normally on Windows + 1.0.1 + Python 3.13). plus.7.4. |
