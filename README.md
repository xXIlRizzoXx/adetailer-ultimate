...
| 🟡 | Apply only on hires.fix | ADetailer always runs on every `postprocess_image` call, including the lowres txt2img call that precedes the hires.fix upscale — wasted compute since the hires step overwrites the lowres detail. | Per-tab checkbox (`ad_apply_on_hires_only`) that skips the tab's pass during the lowres pre-hires call and only runs ADetailer once the hires.fix upscaled image is ready. In img2img or when hires.fix is off, the tab is skipped entirely (the user explicitly asked for "hires only"). Decision matrix in helper `_should_skip_for_hires_only(p, args)`. Inspired by [Anzhc/aadetailer-reforge](https://github.com/Anzhc/aadetailer-reforge). |
