...
| 🟢 | Pick which detections to inpaint | No way to keep only specific detected instances — every match that passes the class/confidence/size filters is inpainted, and there's no per-instance control. | New per-tab **"Inpaint only these detections"** text field in the **Detection** section: type the detection numbers to keep (e.g. `1,3,5`, or a range `1-3`); blank inpaints all. The **Detection preview** now labels each box with its number (`#1`, `#2`, …), so it's a two-step flow: preview → type the numbers → Generate. Also works in **Run ADetailer on an image**, where the "pause" is just the gap between previewing and running. The numbers follow the raw detector order and only line up when detector + confidence + detection resolution match between preview and run (so it's most reliable in the two-step / standalone flow; a true mid-generation pause isn't possible in the WebUI). Requested in #5. Index-safe: one new per-tab field on the uniform `.change` loop + a pure runtime filter (`filter_by_indices`) run first in `pred_preprocessing` + preview numbering; no new event handlers. |
