...
| 🟢 | Preset live preview (`[SEP]` / `[PROMPT]` aware) | Selecting a preset from the dropdown gives no hint of its contents until you commit by clicking Load. | A markdown preview block under the preset dropdown updates on every `preset_dropdown.change` event with the preset's detector, classes, prompts, sequential flag, and class-specific prompt summary; Save and Import also refresh it when they replace the preset it shows. `[SEP]` and `[PROMPT]` tokens are wrapped in backticks and a footnote reminds the user they'll be expanded at generation time. Formatter `_format_preset_preview`. |
...
| 🟢 | AUTOMATIC1111 preset "Export" button — Gradio 3 | The preset-library Export button uses `gr.DownloadButton`, which only exists in Gradio 4 (Forge); A1111's Gradio 3 lacks it, crashing the ADetailer tab at build (#2). | Falls back to a plain `gr.Button` on Gradio 3 so the tab builds; export just isn't a one-click download there (Import still works). plus.5. In the local plus.7.5.beta.1 review, the button keeps its label and the status line and the button's tooltip say that export needs Gradio 4 and that `user_presets.json` can be backed up instead (previously the label turned into a server file path and the status claimed a successful export). |
...
A class name the detector does not have (for example `hands` from a preset, pasted parameters or the API, for a model whose class is `hand`) gets no pass of its own and is named in the console, as in single-pass mode; otherwise its pass would detect every class and repaint the regions of the other passes with that name's prompt. When no selected name is known to the detector, one pass inpaints every class with the tab prompt, as with an empty selection (earlier releases ran one such pass per name, each with that name's prompt).
