...
- **Names from a preset, pasted parameters or the API** match regardless of case (`Face` selects `face`; an exact match wins), also in the CLASSES dropdown when loading or pasting changes the detector. A name the detector does not have is ignored and named in the console; when no name of an include filter matches, every class is inpainted, as with an empty selection. The class names of the per-class prompt lines match regardless of case too.
