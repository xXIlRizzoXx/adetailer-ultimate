...
| 🟢 | MediaPipe "face features" detector | MediaPipe only offers whole-face / eyes-only masks; there is no way to isolate the mouth, nose or eyebrows, or to pick which facial parts to detail. | New detector **`mediapipe_face_features`** that treats the face as a **multi-class** model — classes **eyes, mouth, nose, eyebrows, face** — with landmark-precise masks. It reuses the existing **CLASSES** dropdown, so you pick **all** (empty = all), **some**, or **one**; include/exclude (NOT) mode, **Process classes sequentially** (separate pass + prompt per part), **per-class prompts** and **Auto class-guard** all work because it emits real per-mask class names. Runs on MediaPipe's current `tasks` API (`FaceLandmarker`) with a `solutions` fallback for older installs and a hard-coded nose fallback, degrading to empty on any failure — see the `tasks`-API migration row under Cross-WebUI compatibility. The stock `mediapipe_face_mesh_eyes_only` is untouched. Detector `mediapipe_face_features` in `adetailer/mediapipe.py`; class list `FACE_FEATURE_CLASSES` in `adetailer/classes.py`. Inspired by [djunk1159/ADetailer_mediapipe_mouth_only](https://github.com/djunk1159/ADetailer_mediapipe_mouth_only). |
