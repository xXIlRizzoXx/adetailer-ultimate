...
| 🟢 | HD detection resolution (per-tab) | The YOLO detector always runs at Ultralytics' default inference size of 640px, so small or distant faces/parts are often missed, with no control to raise it. | New per-tab **"Detection resolution"** slider (0–1536, step 64, default 0) in the **Detection** section. 0 keeps the 640 default; a higher value (e.g. 1024) passes `imgsz` to `ultralytics_predict` so the detector runs at that resolution and catches smaller/more distant subjects (at the cost of VRAM/time). Boxes are rescaled to the original image internally, so masks/crops/inpaint are unchanged. Applies to the generation pass and to **Run ADetailer on an image**; the quick Detection preview stays at the default resolution in this build. Off by default, index-safe, backward-compatible. |
