...
- **New: built-in guide inside the extension — now a dedicated top-level tab.** A **"📖 ADetailer Guide"** tab appears in the main tab bar (next to Settings) with a **table of contents** and one **collapsible section per topic** (Getting started, Choosing a detector, Detection, Mask preprocessing, Prompts, Inpainting, Per-pass overrides, Tools, Presets, Fixing common problems, Special tokens, What's new) — a complete plain-language reference for every option, no need to open GitHub. A small non-invasive pointer at the bottom of the ADetailer panel links to it. Registered via the core `on_ui_tabs` callback (guarded — a failure just yields no tab) as its own `gr.Blocks`, so it's fully isolated from the panel wiring and index-safe; read-only English reference text.
