...
- **New: "active tabs" counter pill in the accordion header.** A small green pill (e.g. `2x Tabs`) next to the "ADetailer" title shows how many detector tabs are active — i.e. how many "Enable this tab" checkboxes are ticked — and updates live as you toggle them; it stays visible when the accordion is collapsed and hides at 0, à la ControlNet Integrated's unit counter. Pure client-side (`javascript/tab-count-pill.js` + CSS), so it adds no Gradio component or listener — index-safe. Guarded writes (only on change) keep the DOM observer from busy-looping.
