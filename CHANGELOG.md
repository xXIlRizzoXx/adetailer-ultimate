...
- **On a console in a legacy code page, five more console messages could stop ADetailer, and one of them its panel.** As for the "inpaint indices" warning, a dash that Japanese and Korean code pages lack made printing fail when the WebUI's output goes to a file or a pipe: the line that reports a tab restored from the remembered settings then stopped the ADetailer panel from being built at every start, the line that reports a saved tab raised on every Generate, a rolled-back sequential pass skipped the image's later tabs, and the settings reset lost its status. These messages are now plain ASCII. Earlier releases had the same problem.
