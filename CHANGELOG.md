...
- **Cancelling a pass could keep a half-finished region.** Pressing Interrupt or Skip while a region was being inpainted could leave that half-denoised region in the final image, and a standalone or folder run saved it as detailed. With two or more detections this was new in this beta; for the last or only region (for example a single face) it also happened in earlier releases. The cancelled pass is now always discarded, and a standalone run reports it as cancelled instead of "nothing detected". **Skip now stops ADetailer for the rest of the current batch** — previously it only cut the current region short and the remaining regions were still inpainted.
