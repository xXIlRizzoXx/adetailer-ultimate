...
- **Click the result to enlarge it full-window.** The result is shown in a gallery (like normal generated images), so clicking it expands to fill the window — on both Forge Neo (Gradio 4.40) and A1111. (koblue's request in #4; the earlier `gr.Image` fullscreen button doesn't exist on Forge Neo's Gradio 4.40, so the result box is a `gr.Gallery` with `allow_preview` instead.)
