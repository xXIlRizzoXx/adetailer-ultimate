...
## v26.2.0+plus.8.beta.1 — first beta (reliability update), 2026-09-23

First beta of the plus.8 reliability update, published on GitHub as a **pre-release** for testing. It is not a stable release: the stable version is still v26.3.0+plus.7.4. Version numbers now start with the version of the upstream ADetailer this fork is built on: 26.2.0, the latest release of Bing-su/adetailer. Releases up to v26.3.0+plus.7.4 used 26.3.0 instead, so by number this beta sorts before them although it is newer. It fixes problems found by two independent reviews and a debugging pass in several rounds across detection, class filtering, presets, pasting, the UI and the generation pipeline, most of them present in earlier releases too; every fix from the reviews and the debugging pass comes with a regression test. To try it, switch the extension's git clone to the `beta` branch (`git fetch origin`, then `git checkout -B beta origin/beta`, which also replaces an older local `beta` branch; `git fetch origin` and `git checkout -B main origin/main` go back to the stable version) and restart the WebUI; the same code is tagged `v26.2.0+plus.8.beta.1` on the Releases page. It is being tested on AUTOMATIC1111 1.10 and Forge Neo, so please report any problem in the Issues. In the entries below, "the first beta build" means the first, unpublished build of this update that the reviews examined, and "Earlier releases had the same problem" means that plus.7.4 had it too. Later betas get their own section, listing only their own changes. Known issues are listed at the end of this section.
...
- Add focused regression coverage, MediaPipe face-features and platform-guard tests, and a Windows unit-test job. Trigger checks for all Python changes, the front-end scripts, the stylesheet, the README, the changelog and dependency configuration, including pushes to main. Mark model-download tests separately and bound test-image HTTP requests.
...
**Fixed after an independent review of this beta** (each with a regression test that fails before the fix):

- **"Reset every tab" reset only the first tab on Forge, Forge Neo and reForge** (a regression of the first beta build; plus.7.4 was not affected). The reset built one set of values and handed the same objects to every tab; Gradio 4 consumes those values as it applies them, so every tab after the first kept its old settings while the status still read "All tabs reset". Each tab now gets its own values. AUTOMATIC1111 (Gradio 3) was not affected. Verified against the real Gradio 4.40 code.
- **Cancelling a pass could keep a half-finished region.** When Interrupt or Skip cut a region short, that half-denoised region could stay in the final image, and a standalone or folder run saved it as detailed. For Skip, and for the last or only region (for example a single face), this also happened in earlier releases; for Interrupt on an earlier region it was new in the first beta build. A pass cut short is now always discarded, also when the last region fails with a NaN error after the cancel. A standalone run reports it as cancelled instead of "nothing detected" (including when AUTOMATIC1111's default Interrupt, "stop after the current image", stops the pass between regions), and a folder run lists cancelled files separately. **Skip now stops ADetailer for the rest of the current batch** — previously it only cut the current region short and the remaining regions were still inpainted.
- **Interrupt did not stop a folder run on AUTOMATIC1111.** With A1111's default "interrupt after current" option, Interrupt only asks the host to stop after the current image; the folder ignored that request and went through every remaining file without detailing any of them. The folder now stops before the next file.

**Fixed after a second independent review** (each with a regression test):

- **A preset or settings file that could not be read was replaced by the next save.** If `user_presets.json` or `user_state.json` had been re-saved in another text encoding, or had a JSON syntax error, it was read as empty and the next save overwrote it, losing every preset or every other tab's settings; the first beta build made this happen for more encodings. The unreadable file is now kept under a new name (`user_presets.unreadable-<date>.json` or `user_state.unreadable-<date>.json`) before the save; for presets the status line says so, for settings the console does. A file that is only temporarily locked is left untouched, and a byte-order mark at the start of either file is accepted.
- **Pasting generation parameters could keep the tab's previous class filter.** PNG Info and Send to txt2img/img2img updated the hidden class fields but not the visible class selection, so when the detector stayed the same the old selection was written back before generating. Missing class keys, which the parameters leave out when they hold their defaults, also left the previous values in place. A tab's class filter, NOT mode and visible selection are now pasted together, and a missing key means its default; fields listed in "Disregard fields from pasted infotext" are left as they are.
- **A sequential tab stopped by AUTOMATIC1111's "stop after the current image" was kept as complete.** The classes done before the stop stayed in the image and a standalone run reported success. The tab is now rolled back like any other cancelled sequential pass, unless the pass for its last class had already been applied.
- **A very early Interrupt on Forge Neo was reported as a failure.** When the cancel arrived before the first sampling step, Forge Neo raised an error on the empty result; the standalone tool showed a failure and a folder run counted an unreadable file. It is now treated as the cancel it is.
- **Standalone runs under-counted their work by one** in the WebUI's job counter, which the progress API and AUTOMATIC1111's "stop after the current image" rely on.
- Hosts whose job start does not accept a job label are handled.

**Fixed during the final debugging pass** (each with a regression test):

- **Pasting generation parameters left a pasted tab switched off.** Tabs 2 and later start disabled, and the parameters never include "Enable this tab", so PNG Info or Send to txt2img/img2img filled in the second tab's detector and prompts but the second pass never ran. A tab whose detector is in the pasted parameters is now switched on. An empty detection-number filter ("inpaint indices") is left out of the parameters too, so an old filter stayed in place and only some detections were inpainted; it is now cleared. As with the class filter, "ADetailer tab enable" and "ADetailer inpaint indices" no longer appear among the choices of "Disregard fields from pasted infotext" (a choice already made is still respected).
- **Loading a preset, pasting a tab or pasting generation parameters replaced the ControlNet preprocessor.** When the ControlNet model changed, its preprocessor was reset to the model's first one (on Forge and Forge Neo, to "None"), so the next image used a different preprocessor than the one saved. A preprocessor that fits the model is now kept. Pasted generation parameters leave the preprocessor out when it was "None"; a missing preprocessor now pastes as "None" (on AUTOMATIC1111, where "None" is not in the list, the model's first preprocessor, as when choosing a model) instead of keeping the tab's previous one.
- **A folder run without a detector said every image was unreadable.** "Run ADetailer on an image" with a folder now reports "Pick a detector model first." (or the settings problem) without opening every file. A run that fails, for example with CUDA out of memory, is counted as failed with its reason instead of "skipped (unreadable)", and a batch where nothing succeeded is headed "Batch failed" instead of "Batch done".
- **Preset import refused a file with a byte-order mark** (as saved by some Windows editors and PowerShell 5.1), although the library file itself accepts one.
- **A preset named "(none)" could be saved, imported or renamed to, but then never loaded, renamed or deleted,** because "(none)" is the dropdowns' "no preset selected" entry. The name is now refused. An existing preset with that name has to be removed from `user_presets.json` by hand.
- **On AUTOMATIC1111 (Gradio 3), Export replaced its button label with a server file path and reported a successful export,** although nothing was downloaded. The button now keeps its label and the status explains that export as a download needs Gradio 4 (Forge / Forge Neo), and that `user_presets.json` in the extension folder can be backed up instead.
- **A translated class name could turn the class filter off.** When the WebUI's localization translated a selected class name in the dropdown, the in-browser class sync wrote the translated word, which matches no class, so every class was inpainted and the wrong value was saved. The sync now leaves such a tab to the server-side sync, which uses the real names.
- **On the light theme, the Inpaint prompt placeholders, the preset-name placeholder and the "Preset library" heading were white on white.** The white text now applies to the dark theme only.
- **A repeated identical status message stayed hidden.** The preset and import/export status fades after about 4 seconds; the same message sent again (for example a retried Save that failed again) was not shown. It is now shown again.
- **On AUTOMATIC1111, the version badge and the "📖 Guide" link were positioned twice** and ended up above the ADetailer header, clipped or over the control above. They now sit on the header as on Forge.
- **The "Reset every tab" checkbox showed the Reset button's "Reset this tab…" tooltip.**
- **Reset switched "Enable this tab" on for tabs 2 and later,** unlike a fresh setup, so the header counter counted every reset tab. Reset now leaves only the first tab enabled.
- **On AUTOMATIC1111, a class filter was ignored for a detector without a class-name sidecar.** AUTOMATIC1111's safe-unpickle check refuses to read an Ultralytics `.pt` while the UI is built or a detector is chosen, and that empty class list was then kept for the whole session, also for generation, which is allowed to read the file. An include filter restored from a preset, pasted parameters or the API therefore inpainted every class, NOT mode excluded nothing, sequential mode ran every class once per listed class, inline `[CLASS=…]` blocks were dropped and the Auto class-guard did nothing. Only class names that were actually found are now kept: generation reads the real names and the UI reuses them afterwards (switch to another detector and back to see them in the CLASSES dropdown, or add a `<model>.names.json` to see them straight away). The refused UI lookup is not repeated, so the host's warning still appears only once per model. Classic Forge and reForge keep AUTOMATIC1111's check and should behave the same (not tested); Forge Neo was not affected.
- **YOLO-World regions were named "0", "1", … instead of by their class.** Inline `[CLASS=person] … [/CLASS]` blocks were therefore dropped, and `[CLASS=person] [SKIP] [/CLASS]` inpainted the region with an empty prompt instead of skipping it. Regions now carry the class typed in the CLASSES field (or the model's own class name when the field is empty), which also shows in the combined Detection preview and the verbose log.
- **"Exclude selected (NOT)" was shown for YOLO-World but had no effect there.** YOLO-World always detects the typed classes, so ticking it inpainted exactly the classes meant to be excluded, and it also turned sequential mode off. The checkbox is now hidden and off for YOLO-World detectors, also after loading a preset, pasting a tab or generation parameters, and at startup; it comes back when another detector is chosen.
- **Merge and Invert could repaint the whole image.** When the detections covered the whole frame (for example a close-up whose box is clamped to the image edges, or several boxes side by side), the inverted mask was blank. The WebUI treats a blank mask as "no mask" and regenerated the entire picture, subject included, at the ADetailer denoising strength, possibly at another size. Nothing is left to inpaint in that case, so the pass is now skipped.
- **A `<model>.names.json` saved by Windows PowerShell was ignored or broke the tab.** With a UTF-8 byte-order mark the file was silently ignored, so a class filter matched nothing and every class was inpainted; as UTF-16 (what PowerShell 5.1 writes with `>`) it raised an error when choosing the detector, loading a preset or generating with a class filter. Class-name sidecars in UTF-8, with or without a byte-order mark, and UTF-16 are now read; a file that still cannot be decoded is skipped like a malformed one.
- **On Forge and Forge Neo, "Automatic" as the tab's separate VAE kept the base model's VAE** (a regression of the first beta build; plus.7.4 was not affected). The beta's rule that keeps the base module when a chosen file is missing also caught the dropdown choice "Automatic", so the detailer checkpoint no longer used its own VAE. With a base model that loads a separate VAE of another architecture (for example a Flux or Z-Image `ae.safetensors`) and an SDXL detailer checkpoint, the detailer pass failed. "Automatic" again drops the base VAE, and the misleading "not found in Forge's module list" warning is no longer printed for it. The rule for missing files is unchanged.
- **Cancel in the "Reset ADetailer settings" confirmation still reset and saved every ADetailer option.** Gradio sends the request whatever the confirmation script returns, so the options were reset and `config.json` saved while the page kept showing the old values. Cancel now stops the request in the browser; OK works as before.
- **With Skip img2img, "Use same sampler" ran the detailer with Euler, and the saved image's parameters said "Steps: 1, Sampler: Euler, Size: 128x128".** Those are the values of the throwaway one-step pass. "Use same sampler" now uses your sampler, and the saved parameters (PNG Info, Send to) record your steps, sampler and size. The throwaway pass itself is unchanged, so Batch count above 1 keeps working.
- **"Run ADetailer on an image" replaced the WebUI's record of your last generation.** Its inner detailer pass rewrote `params.txt`, so the paste button with an empty prompt filled the UI with a blank prompt, 28 steps and a random seed you never used. The record is now kept, for a single image and for every file of a folder run.
- **"Run ADetailer on an image" varied only the first region.** The first region got a random seed and every other region the fixed seeds 0, 1, …, so pressing Run again changed only one face. Each run now picks a random seed for all regions (and "Use same seed for each tab" shares it between them).
- **The Auto class-guard was switched off by a per-class prompt line outside sequential mode.** Per-class prompts only apply to a sequential pass with two or more classes, but a line for a class (for example one left over after turning sequential mode off) disabled the guard for that class in every mode, with nothing in its place. It now only takes priority in a sequential pass.
- **Merge and Merge and Invert gave their region the first detection's class.** A merged mask holding a face and a hand got "face" from the Auto class-guard and "hand" in its negative prompt, and a Merge and Invert background got "face" in its positive prompt. Inline `[CLASS=…]` blocks followed the same wrong class. A merged region that mixes classes now gets no guard and keeps the inline blocks of the classes it holds; a `[SKIP]` block skips it only when every class it holds is skipped, and a block for a class it does not hold (for example eyes, on a merged face and hand) neither changes nor skips it. A Merge and Invert region, the background, gets no guard and no class block, for class-less detectors too, so `[CLASS=hand] [SKIP] [/CLASS]` no longer skips it. Two merged faces are still a face.
- **Merge and Invert sized the inpaint from the detections instead of the inverted region.** With resolution scale or the global bbox-size matching, the whole background was squeezed into a canvas the size of the detected faces and pasted back blurred. The canvas now follows the inverted region; dynamic denoise still follows the detections. With resolution scale the canvas is now the inverted region's size times the scale, so it needs more memory than before.
- **An empty "ADetailer class prompts" was written into every image's parameters.** On AUTOMATIC1111 (and probably classic Forge and reForge) pasting them printed `Error parsing "ADetailer class prompts: "` and kept the tab's old class prompts. An empty or blank field is no longer written, and pasting parameters that have a tab's detector but no class prompts clears that tab's field, also for images saved before this fix, unless the field is listed in "Disregard fields from pasted infotext".
- **After a NaN error on one region, the next region's dynamic denoise was applied twice** (AUTOMATIC1111, classic Forge and reForge; Forge Neo never raises this error), so it was inpainted too weakly. Each region now starts from the tab's own values.
- **A negative global "Power scaling for dynamic denoise strength" stopped ADetailer on a region that fills the image.** The formula divided by zero, and the host dropped that tab and every later one for the image. Such a region now keeps the unmodified strength.
- **Pasting generation parameters left ADetailer itself switched off.** The parameters never include "ADetailer enable", so PNG Info or Send to txt2img/img2img switched the image's tabs on while the ADetailer checkbox stayed off, and the image was made again without ADetailer. Parameters with an ADetailer detector now switch it on, unless "ADetailer enable" is listed in "Disregard fields from pasted infotext". The API's `infotext` field is not affected. Skip img2img is still not restored from pasted parameters.
- **Pasting generation parameters kept tab settings the image did not use.** The parameters leave out settings that hold their defaults (an empty ADetailer prompt, an unticked "Use separate steps / CFG scale / checkpoint / VAE / sampler / …", offsets, top-k, restore faces, ControlNet, …), and the WebUI keeps a field whose key is missing, so a tab kept, for example, its old prompt or separate checkpoint and the next image differed. For every tab whose detector is in the parameters, a missing setting now means its default; a value that only applies while its "Use separate …" box is ticked is left as it is. A tab that is not in the parameters of an image made with ADetailer is switched off (its settings are kept). Fields listed in "Disregard fields from pasted infotext" are left alone. The `parameters` of the `/sdapi/v1/png-info` API now list these defaults too.
- **On Forge and Forge Neo (Gradio 4), preset Export downloaded nothing on the first click while the status said it had worked, and later clicks downloaded the previous click's file,** which missed presets saved in between. Gradio's download button downloads the file it already holds when clicked, before the new one is written. Each click now downloads the file it has just written, once, and the status comes from the same step. Not yet tested in a browser.
- **YOLO-World kept a hidden excluded-class list from the previous detector.** NOT mode does not apply to YOLO-World and is switched off there, but the old list (for example "hand") was still written into the image's parameters as "ADetailer model classes excluded" and shown in the preset preview, and loading or pasting carried it on. Detection was not affected. The list is now emptied for YOLO-World, also when loading a preset, pasting and at startup.
- **After a restart, a restored ControlNet model's preprocessor was hidden,** with only "None" in its list, and choosing the model again to show it reset the preprocessor. The saved preprocessor is now shown with the model's choices at startup; one that does not fit the model falls back to the model's first, as when choosing a model.
- **A folder run with "Save results in the source folder instead" said "see console" for results it could not write, but printed nothing there.** The console now names the file and the reason (for example a full disk or a folder you cannot write to, or no free `-ad` name).
- **A power cut or system crash shortly after a save could leave `user_presets.json` or `user_state.json` empty.** Windows can record the replaced file before its contents reach the disk; the next start then found no presets or remembered settings, and the next Save set the empty library aside and kept only the new preset. Both files are now on disk before they replace the old ones (where the file system supports it).
- **The button tooltips were always in English,** although the Language Diffusion dictionaries translate them: the WebUI translates a tooltip only when its button appears, before this extension sets it. Tooltips now use the dictionary's translation when there is one. The dictionaries still have the old wording of the Copy settings and Reset tooltips, so those two, and the new Export tooltip shown on Gradio 3 WebUIs, stay English until the dictionaries are updated; so does the reworded "Reset ADetailer settings" help text on the Settings page.
- **On a translated UI, the Paste settings button went back to English after Copy or Reset.** Gradio changes the button's text in place, which the translation patch did not watch; it is now translated again.
- **On the light theme, the Detection preview and Run ADetailer status text was amber on a near-white background** and barely readable. The amber text now applies to the dark theme only; the light theme uses its normal text colour.
- **On Windows, every MediaPipe detector found nothing when the WebUI's folder path had a non-ASCII character** (for example an accented, Cyrillic or CJK user name). MediaPipe could not open its model file by that path, so the file was deleted as corrupt and downloaded again on every detection, and a file placed there by hand was deleted too. The model file is now read by the extension and handed to MediaPipe as data. Earlier releases had the same problem.
- **A class name in a different case, such as "Face" for the model's "face", switched the class filter off.** An API call, a preset or pasted parameters could send such a name; it matched no class, so every class was inpainted, and in NOT mode nothing was excluded. Class names now match regardless of case (an exact match still wins), and a name the detector does not have is named in the console. When no name of an include filter matches, every class is still inpainted, as before. Earlier releases had the same problem.
- **A region that the `[CLASS=…]` blocks left with no text of its own was inpainted with an empty prompt.** A blank ADetailer prompt means the main prompt, but `[CLASS=hand] [SKIP] [/CLASS]` alone, the documented way to skip only the hands, left the face with an empty prompt (or only the append text and the main prompt's LoRAs, when set); so did `[CLASS=face] [PROMPT], smile [/CLASS]` for the hands, and a negative prompt made only of blocks did the same. A region that no block matches, when there is no text outside the blocks, now gets the main prompt exactly as a blank prompt does; so do a Merge and Invert background and a merged region whose only matching block is a `[SKIP]` it does not follow. A region with a matching block keeps only its text, as before: `[CLASS=hand] five fingers [/CLASS]` still gives the hands "five fingers" alone, and `[PROMPT], [CLASS=hand] five fingers [/CLASS]` adds the main prompt. Earlier releases had the same problem.
- **On Forge and Forge Neo, the detailer pass ignored the Distilled CFG Scale / Shift** and always used the host's default, 3.5, while the main image used your value (Forge Neo's slider starts at 3.0, and its presets set other values for models such as Z-Image, Lumina or Wan). The Flux guidance or sampling shift of every detailed region therefore differed from the rest of the picture. The detailer pass now uses the main image's value. With "Use separate checkpoint" it keeps the host default, 3.5, because a checkpoint of another model family reads the value as a different setting (Flux guidance instead of sampling shift, or the other way round): Forge Neo's SDXL preset, for example, sends a hidden shift of 9, which a Flux detailer checkpoint would use as its guidance. AUTOMATIC1111 has no such setting and is not affected. Earlier releases had the same problem.
- **With "Inpaint only masked" off, "Scale inpaint to bbox" and the global bbox-size matching resized the whole final image** to the region's canvas: a small stretched thumbnail with the scale, a changed aspect ratio with "Free" or "Strict". They now apply only with "Inpaint only masked"; when the whole picture is inpainted, the canvas keeps its size. Earlier releases had the same problem.
- **On Forge Neo, with "Inpaint only masked" off, the second region failed** ("Only uniform Scaling is supported" or "Inpaint only supports Scale divisible by 0.25") whenever the inpaint canvas differed from the image, for example with "Run ADetailer on an image" on a 1536x1024 picture or with "Use separate width/height", and the host dropped every fix of that tab. The host returns each region at the canvas size, so the next region's mask is now resized to match it. AUTOMATIC1111, classic Forge and reForge already accepted such a mask, but softened its edge (the mask blur) before resizing it; from the second region on they now soften it at the canvas size, so with a mask blur above 0 that edge is wider than before when the canvas is smaller than the image (as with "Run ADetailer on an image" on a picture larger than 1024 pixels) and narrower when it is larger (as with a larger "Use separate width/height"), by the ratio of the two sizes; the first region is unchanged. Earlier releases had the same problem.
- **The Auto class-guard of `mediapipe_face_features` put the face's own parts in its negative prompt** ("eyes, mouth, nose, eyebrows" for the face region, which holds them) and "face" in every part's negative prompt. The face region now gets no negatives and "face" is never a part's negative; the parts still guard against each other. Earlier releases had the same problem.
- **The XYZ grid's "[ADetailer] Prompt S/R" axes logged an error for every grid cell** ("Error running process"), and with a batch size above 1 some images could be saved without their ADetailer parameters. The host numbers the images only after the step that writes those parameters; the first image's number is now used there. Earlier releases had the same problem.
- **Dynamic denoise could stop ADetailer on merged MediaPipe faces.** MediaPipe face boxes are not clipped to the image, so with Merge or Merge and Invert the box around two faces cut by opposite edges could be larger than the frame; the formula then gave a complex number (an error, and the host dropped that tab and every later one for the image), a negative strength or one above the tab's own. Such a box now counts as the whole image. Earlier releases had the same problem.
- **"Run ADetailer on an image" said "Nothing detected" when every detected region failed with a NaN error** (AUTOMATIC1111, classic Forge and reForge; Forge Neo never raises this error), and a folder run counted such files as "left unchanged (nothing detected)" under "Batch done". It now reports "ADetailer run failed: NaN error while inpainting.", and a folder run counts the file as failed.
- **Loading a preset saved by an older version kept the tab's current values for every setting the preset did not have.** A preset saved before plus.7.3 has no "Inpaint only these detections" value, and one saved before plus.7 also lacks the detection resolution, the Auto class-guard, the per-tab dynamic denoise and other settings added since. Loading it said "Loaded" but left those fields as they were, so a leftover detection-number filter such as "2" inpainted only that detection, or a class guard the preset never had stayed on. Those settings now go back to their defaults, as the plus.7 notes promised; "Enable this tab" is left as it is, and so is a separate checkpoint, VAE or text encoder choice the preset does not name (a preset saved before plus.5 has no text encoder): its "Use separate …" checkbox goes back to off, so the choice has no effect. Earlier releases had the same problem.
- **Save, Delete and Rename changed the preset selected in every other tab.** Saving a preset in one tab selected it in all of them, so a Delete or Load in another tab then acted on that preset instead of the one chosen there, and an error such as Save without a name, or Delete with nothing picked, reset every tab to "(none)" although nothing had changed. Every tab still gets the new list of presets, but only the acting tab changes its selection; a tab that showed a renamed preset follows the new name, and one that showed a deleted preset is cleared. Earlier releases had the same problem.
- **A class name in another case still switched the class filter off in the UI.** When loading a preset or pasting parameters changed the detector, the detector change that follows kept only exact names, so "Face" for the model's "face" was dropped from the CLASSES dropdown and the filter, and every class was inpainted (in NOT mode, nothing was excluded). The UI now matches such a name as generation does (an exact match wins, and a name that matches several classes is dropped) and selects the model's own name. Earlier releases had the same problem.
- **Typing a detection number as the Detection preview draws it, such as "#2", or separating the numbers with spaces ("2 3"), inpainted every detection.** "Inpaint only these detections" found no number it could read in such a value and, as for a blank field, kept every detection without any message; "1, #3" kept only the first one. A "#" is now ignored and spaces separate numbers like commas ("1 - 3" is still the range 1-3). A value with no number at all still inpaints every detection, and the console now says so. Earlier releases had the same problem.
- **The ADetailer Guide tab said that folder-batch results are always saved to the `ADetailer-Inpaint` folder and that the preset library can be exported as a file.** It now describes "Save results in the source folder instead" (results beside each source image as `name-ad`) and says that on WebUIs with Gradio 3 (AUTOMATIC1111, reForge's main branch) Export cannot download a file, so `user_presets.json` has to be backed up instead. The README's folder-batch paragraph names both destinations too.
- **With "Inpaint only masked" off, a Hires fix image was shrunk back to the first-pass size.** The host keeps the generation's width and height at the first pass, and in this mode it resizes the whole image to that canvas, so a 1024x1536 result of Hires fix x2 came back at 512x768 and the hires pass was lost. The canvas is now the upscaled image's own size (unless "Use separate width/height" is set), which needs as much memory as the hires pass itself. Earlier releases, and the original ADetailer, had the same problem.
- **The before, mask-preview and step images of a batch recorded the first image's seed and prompts.** With Batch count or Batch size above 1, the `-ad-before`, `-ad-preview` and `-ad-step` files of the second and later images had their own seed in the file name but the first image's seed, prompt and negative prompt in their parameters, so PNG Info or Send to txt2img brought back the first image. Each file now records its own image. Earlier releases had the same problem.
- **A tab that failed with an error left the detailer pass in `params.txt` and stopped other scripts for the rest of the batch.** When a tab raised (for example a detector named by the API or a preset that is not installed, or CUDA out of memory), the host logged the error and went on, but the paste button with an empty prompt then filled in the detailer pass's prompt and size. ADetailer also finishes the other scripts before its first pass of a batch and starts them again after its last one; after an error they were not started again, so on Forge Neo the built-in ControlNet logged an error and was left out of every later batch. Both steps now run whatever the outcome, and the error is still reported. Earlier releases had the same problem.
- **"Run ADetailer on an image" ignored the global "Output directory for images".** With that setting (Settings → Paths) the WebUI saves every generation in one folder, for example on another drive, but "💾 Save result to outputs", folder batches and the tool's step and preview images still went to the WebUI's own outputs. The `ADetailer-Inpaint` folder is now created inside that folder, and the step and preview images go there too. Without the setting nothing changes. Earlier releases had the same problem.
- **With Skip img2img, an API request with several init images detailed the first one for every output.** The host makes each output from its own init image, but ADetailer replaced every output with a detailed copy of the first, so the other init images were lost and their outputs showed the first image under their own seed. Each output now details its own init image. The img2img tab, including its Batch tab, uses one image for every output and is not affected. Earlier releases had the same problem.
- **"Run ADetailer on an image" said "Nothing detected" when it had found regions but none was left to inpaint,** for example when "Inpaint only these detections" named numbers that do not exist, the mask filters dropped every region or every region was skipped with `[SKIP]`, although the Detection preview showed them. It now says "Detections found, but the detection numbers, mask filters or [SKIP] left none to inpaint", and a folder run counts such files as "left unchanged (nothing to inpaint)". Earlier releases had the same problem.
- **Per-class prompt lines matched their class only in the exact case.** With a class filter such as "Face,Hand" from a preset, pasted parameters or the API and lines `face: …` / `hand: …`, or with a line typed `Face:` for the model's "face", a sequential pass ignored the line, applied neither it nor the Auto class-guard, or applied both. A line's class name now matches regardless of case, as the class filter does (an exact match wins), and the guard follows the same match. Earlier releases had the same problem.
- **Pasting the parameters of an image made with the original ADetailer kept this fork's own toggles.** Such an image does not record "Apply only on hires.fix", "Use bbox as mask" or "Scale inpaint to bbox", which this fork always writes, so a tab that had them on kept them: ADetailer was then skipped without Hires fix, or used box masks and a scaled canvas, and the pasted image did not come out the same. They are now switched off like the other settings the parameters leave out; the scale value itself is kept, as it only applies while its toggle is on. Images made by this fork record them and are not affected.
- **The console error report broke when a tab's prompt used `[CLASS=…] … [/CLASS]` blocks.** When ADetailer failed (for example a missing detector or CUDA out of memory), the report that lists the system info, the tab's settings and the traceback read the prompt's `[/CLASS]` as formatting and failed itself, so the console showed a "MarkupError" about `[/CLASS]` instead of the real error. Prompts are now shown in the report exactly as typed. Earlier releases had the same problem.
- **A folder run that saved nothing was still headed "Batch done"** when every result was detailed but none could be saved (for example a full disk or a folder you cannot write to), and so was a run whose last or only file was cancelled by Interrupt or Skip. They are now headed "Batch failed" and "Batch interrupted", and the "done/total" count of an interrupted run includes the files that failed or could not be read. Earlier releases had the same problem.
- **On the light theme, the "📖 Guide" link in the ADetailer header was barely readable:** light blue, dimmed with the version text, on white (about 1.6:1). It now uses a darker blue at full strength there (about 5:1); the version text looks as before, and the dark theme is unchanged. Earlier releases had the same problem.
- **On WebUIs with Gradio 3 (AUTOMATIC1111, reForge's main branch), the Export button's tooltip promised a JSON file** that the button cannot download there. Its tooltip now says that export as a download needs Gradio 4 (Forge / Forge Neo) and that `user_presets.json` in the extension folder can be backed up instead. Earlier releases had the same problem.
- **On AUTOMATIC1111, a class-name sidecar added while the WebUI was running was ignored until a restart.** Once the safe-unpickle check had refused a detector's `.pt` and its CLASSES dropdown had come up empty, a `<model>.names.json` written afterwards was not read, so no class could be picked in the UI until a restart or a generation with that detector. The sidecar is now read when you switch to another detector and back. The refused `.pt` is still not loaded again, so the host's warning still appears only once per model. Classic Forge and reForge keep the same check and should behave the same (not tested); Forge Neo was not affected.
- **The "inpaint indices" warning for numbers that match no detection could stop ADetailer for the image.** When the console output goes to a file or a pipe in a legacy code page (for example a WebUI started with its output redirected to a log), the message could not be printed: a value such as "5" on a Japanese or Korean system (the message had a dash those code pages lack), or a number next to non-Latin text; the host then reported an error and skipped ADetailer, including its later tabs, for that image. The message is now plain ASCII, like the "no usable number" one. Its dash was in earlier releases too; the non-Latin case became easier to reach when spaces started separating numbers in this beta.
- **With Skip img2img, the img2img Batch tab recorded the first file's size for every file.** The host reuses one generation for the whole folder and, with "Resize by", sets each file's own size before it (with "Append png info", also its own steps and sampler), but ADetailer kept the values it had saved for the first file: a landscape file after a portrait one was saved with the portrait size in its parameters, so Send to img2img/txt2img restored the wrong size. From the second file on, the throwaway pass also ran at the file's full size, and with "Inpaint only masked" off the file was stretched to the first file's size. Each file now records and uses its own settings, also a file made with the Euler sampler or with one step (which were first taken for the throwaway values and given the previous file's sampler or steps), and gets the one-step 128x128 throwaway pass. "Resize to", Batch count and loopback are unchanged. The wrong recorded size came from the Skip img2img parameters fix above (plus.7.4 recorded each file's size, with "Steps: 1, Sampler: Euler"); the other two problems were in earlier releases too.
- **With Skip img2img and "Inpaint only masked" off, the result was resized to the img2img width and height sliders.** The host resized the whole init image to the sliders' size (stretching it when the aspect ratio differs, for example an 832x1216 image with 512x512 sliders), while the same run kept the image's size when nothing was detected or with "Inpaint only masked" on. The canvas is now the init image's own size, rounded down to a multiple of 8 as the WebUI needs (a 1080x1350 image comes back at 1080x1344), unless "Use separate width/height" is set, so the result keeps its size; like a generation at that size, a large image needs more memory than before. Earlier releases, and the original ADetailer, had the same problem.
- **"Use LoRAs from main prompt" added a LoRA again when the tab prompt already had it at another weight.** With `<lora:detail:1>` in the main prompt and `detailed face <lora:detail:0.3>` in the tab prompt, the detailer prompt got both tags, so AUTOMATIC1111 applied the LoRA twice at 1 (the last weight) and Forge Neo at 0.3 plus 1; the same happened for "1" against "1.0". A LoRA the tab prompt already names is now left out, so the tab's weight wins. Names are compared as the WebUI reads them, letter case included. Earlier releases had the same problem. A LoRA or trigger phrase named only inside a `[CLASS=…]` block counts only for that class's regions: the other regions, and a Merge and Invert background, still get the main prompt's one. (The first version of this fix left it out of every region when the block named it at any weight; earlier releases already did so when the block had the same tag or trigger phrase.)
- **In sequential mode, a class name the detector does not have made its pass detect every class.** A filter such as "face,hands" from a preset, pasted parameters or the API, for a detector whose class is "hand", inpainted the faces and then, in the "hands" pass, every face and hand again, with the "hands:" per-class prompt and the Auto class-guard mixed in. Such a name now gets no pass and is named in the console, as in single-pass mode; when no selected name is known to the detector, one pass inpaints every class with the tab prompt, instead of one such pass per name (like every sequential pass, it ignores "Inpaint only these detections"; see also the known issues below). YOLO-World and MediaPipe detectors are unchanged. Earlier releases had the same problem.
- **A folder run in which every file that needed detailing failed or could not be saved was still headed "Batch done" when another file had nothing to inpaint.** For example, with one image without a face and two detailed images that a full disk or a folder you cannot write to refused (or that failed with CUDA out of memory), the status read "✅ Batch done — 3 image(s): 0 detailed …" although nothing was written. Such a run is now headed "Batch failed"; files left unchanged beside unreadable ones still make a "Batch done".
- **A folder run left out images with the `.tif` extension.** Only `.tiff` was accepted, so a folder of scans saved as `.tif` reported "No images found in that folder." and a mixed folder skipped them without counting them. Both spellings are now detailed, as in the WebUI's own img2img batch; with "Save results in the source folder instead" the result is `name-ad.tif`. Earlier releases had the same problem.
- **Reset left the separate checkpoint, VAE and text encoder dropdowns blank.** Their default is empty, which is not one of their choices, so after Reset (or "Reset every tab") the three dropdowns showed nothing, and a preset saved or a tab copied right afterwards carried the empty value on. They now go back to "Use same checkpoint", "Use same VAE" and "Use same text encoder", as on a fresh setup. Earlier releases had the same problem.
- **After saving over the preset a tab shows, or importing a file that replaced it, the preview under the dropdown kept the old contents.** The preview was rebuilt only when the dropdown's value changed, which does not happen when the name stays the same. Save now refreshes the preview of every tab that shows the saved preset, and Import refreshes the preview of its tab. Earlier releases had the same problem.
- **A setting changed from the UI while ADetailer was detailing an image was undone when the pass ended.** ADetailer put back its snapshot of all the WebUI options after every pass, so a Clip skip, checkpoint or other setting changed meanwhile (for example during a long Batch count) was silently reverted: the UI and `config.json` showed the new value, later images used the old one, and the next settings save wrote the old value back. Only the options the pass itself changes are put back now. Earlier releases had the same problem.
- **The "Apply only on hires.fix" help text said that the option has no effect when hires.fix is off,** while the tab is then skipped, and it described a "pre-hires call" that no WebUI makes. It now says "Run this tab only when hires.fix is on, on the upscaled image. With hires.fix off this tab is skipped. No effect in img2img." Translations of the old text show the new text in English until the Language Diffusion dictionaries are updated. Earlier releases had the same problem.
- **On a console in a legacy code page, five more console messages could stop ADetailer, and two of them its panel.** As for the "inpaint indices" warning, a dash that Japanese and Korean code pages lack made printing fail when the WebUI's output goes to a file or a pipe: the lines that report a tab restored from the remembered settings, or a remembered detector that is no longer in the model list, then stopped the ADetailer panel from being built at every start, the line that reports a saved tab raised on every Generate, a rolled-back sequential pass skipped the image's later tabs, and the settings reset lost its status. These messages no longer contain that dash. The saved-tab, restored-tab and missing-detector lines still print the detector and class names as they are (see the known issues below). Earlier releases had the same problem.
- **The console line that names a class the detector does not have could stop ADetailer for the image.** This line, new in this beta, printed the names and the detector's file name as they are, so when the console output goes to a file or a pipe in a legacy code page (as for the "inpaint indices" warning above), a non-Latin class name from a preset, pasted parameters or the API, or a detector file name with such characters, could not be printed: the host reported an error and skipped ADetailer, including its later tabs, for that image. In sequential mode the name also kept a pass of its own, which then failed the same way. The line is now plain ASCII, with any other character written as a backslash escape; ASCII names read as before.
- **"Inpaint only these detections" inpainted every detection when the numbers were typed with Chinese or Japanese punctuation.** An IME in Chinese or Japanese mode types a full-width comma ("1，3"), the list comma "、", full-width digits, or a wave dash or full-width hyphen for a range ("1～3"); the field read no number in such a value, kept every detection and printed that the value had no usable number. These characters, and an en or em dash between two numbers, now work like "," and "-". Earlier releases had the same problem.
- **In an X/Y/Z grid, a cell where an ADetailer axis switched a tab off kept the previous cell's ADetailer parameters in its images.** Every cell shares the parameters of the generation, and ADetailer only added its own: with "[ADetailer] ADetailer model 1st" set to "face_yolov8n.pt, None", the "None" cell, where the tab never ran, was saved with the face detector, prompt and settings of the first cell (and, with this beta's paste fixes, PNG Info or Send to txt2img of such an image switched ADetailer on). The same happened for a prompt or ControlNet model axis whose later value is empty or "None". Each cell now records only its own settings. Earlier releases had the same problem.
- **In the img2img Batch tab with an inpaint mask folder, one empty mask switched ADetailer off for the rest of the batch.** The host reuses one generation for every file; the "no mask" decision for the file with an empty mask (for example from a segmentation tool that found nothing) was kept for every later file, so they were not detailed although their masks were valid. The decision is now made for each file. Earlier releases had the same problem.
- **A per-class prompt line with a `|` inside brackets moved part of its positive prompt into the negative prompt.** The line was split at the first `|`, so `face: {smiling|laughing}, detailed face` gave the positive prompt `{smiling` and the negative prompt `laughing}, detailed face`, steering the face away from what the line asked for; the WebUI's own alternation `[smiling|laughing]` broke the same way. The separator is now the first `|` outside `()`, `[]` and `{}`; lines without brackets, and lines whose brackets do not match, are split as before. Earlier releases had the same problem.
- Some code comments in `style.css` and two passages in older entries of this changelog were not in English; they are now in English.
- README: sequential mode does run for `mediapipe_face_features` (one pass per selected facial part); the README said it was ignored for every MediaPipe detector.
- README: the Sequential class detection row now says that each class pass saves its own mask preview, as the Sequential section does; the Copy/Paste section gives the real Max tabs default and range (4, up to 15), the "📥 Paste from Nth tab" label and the 📂 Load, ✏️ Rename and 🗑 Delete preset buttons.
- README: pasting generation parameters switches tabs off only for an image made with ADetailer (parameters without ADetailer leave ADetailer and its tabs as they are), and the MediaPipe fix for a non-ASCII folder path is no longer described as working "again": earlier releases had the same problem.
- README: the preset export / import accordion is described where it is, at the top of every tab above the preset rows, with its "Overwrite on conflict" checkbox, and Copy / Paste settings below the preset rows; the "Include LoRAs From Main Prompt" section no longer says that a filled tab prompt ignores the main prompt's LoRAs; the localisation notes say that the reworded Copy settings and Reset tooltips and the "Reset ADetailer settings" help text stay English until the dictionaries are updated.
- README: the localisation notes no longer say that every fork label is translated: the options added since plus.7 (Auto class-guard, Strip LoRAs, Detection resolution, Dynamic denoise by area, Inpaint only these detections, the folder batch, Reset every tab, the Guide tab and Verbose diagnostic log settings and others), the Gradio 3 Export tooltip, the preset dropdown's "(none)" entry and the header's "📖 Guide" link stay English until the Language Diffusion dictionaries get them. Earlier releases' README had the same claim.
- README: a class-name sidecar added while the WebUI is running is read only while that detector's CLASSES dropdown is still empty; once names were found (on AUTOMATIC1111, by a generation), adding or changing a sidecar needs a restart. The README promised it without that limit.
- The automatic test run on GitHub now also starts for a change to this changelog alone, which the public-text checks read.

After the review fixes, all 491 offline regression tests pass. The 27 real-detector tests were run again after the detection fixes of the final debugging pass and pass in both the AUTOMATIC1111 and the Forge Neo environments. Before the review fixes, preliminary tests using the installed AUTOMATIC1111 and Forge Neo runtimes, with isolated settings, passed generation with ADetailer, manual mode plus Skip img2img, preset import/copy and standalone detailing. Browser tests in AUTOMATIC1111, also run before the review fixes, passed generation, Send to img2img, Copy/Paste/Reset/Load with excluded classes, standalone detailing, gallery enlargement and folder interruption followed by a successful restart. A generation submitted during the folder run waited in the host queue. Source images and existing results were preserved. Browser tests of the review fixes on AUTOMATIC1111 and Forge Neo, and advanced Forge Neo cross-architecture checks, are part of the testing of this beta; these results do not yet constitute a stable release.

**Known issues in this beta**

- **After Reset, the "ADetailer sampler" dropdown is not put back on the sampler a fresh setup shows.** On WebUIs with Gradio 4 (Forge, Forge Neo) it looks empty; on AUTOMATIC1111 it is set to "DPM++ 2M Karras", which is not one of its choices. The detailer uses this dropdown only with "Use separate sampler" ticked: if you use that option, pick the sampler again after a Reset or "Reset every tab" (a preset saved or a tab copied right after the Reset keeps the wrong value too).
- **"Inpaint only these detections" does not read every character a Chinese or Japanese IME types.** A range typed with the Japanese long-vowel mark (`1ー3`, which the minus key types in Japanese mode) or numbers separated by the Japanese middle dot (`1・3`) are not understood: every detection is inpainted, and the console says that the value has no usable number. A doubled dash, such as `1——3` (what a Chinese IME types for Shift and minus) or `1--3`, inpaints only the first number's detection, without any message. Type a range with a single `-` or `～`, and separate numbers with commas or spaces.
- **On a console in a legacy code page, a detector or class name with non-Latin characters still makes the messages about saved and restored tabs, and about a remembered detector that is no longer installed, fail.** When the WebUI's console output goes to a file or a pipe in a legacy code page (for example a log file on Windows), a detector file name or class name with characters that code page lacks (for example Chinese, Japanese or Cyrillic on a Western system) makes the line that reports a saved tab fail with an error on every Generate, and the lines that report a restored tab or a remembered detector no longer in the model list stop the ADetailer panel from being built at every start. Setting the environment variable `PYTHONIOENCODING=utf-8` for the WebUI, or using names in plain ASCII, avoids it.
- **In sequential mode, the Auto class-guard can be skipped when none of the selected class names exists in the detector** (for example names from a preset, pasted parameters or the API made for another detector). The single pass that then inpaints every class uses the tab prompt, but the Auto class-guard still leaves out every class that has a line in the per-class prompts, although that line is not used in this pass. Remove the unknown names or those per-class prompt lines, or turn sequential mode off, to get the guard back.
...
- **Fix: MediaPipe is pinned to the tested 0.10.x line, so a fresh install can't pull a build that kills the WebUI.** MediaPipe 1.0 (July 2026) is a ground-up rewrite, and **1.0.1** (14 August 2026) rebuilt its detector API on a direct call into the native library that **aborts the whole process** on macOS while a face detector is being created. An abort is not a normal error, so no amount of internal error-handling can catch it — the WebUI simply disappears. Because the extension asked only for "MediaPipe 0.10.13 or newer", **a fresh install was landing on 1.0.1**. It now asks for the 0.10.x line only — `install.py` requests `mediapipe>=0.10.13,<=0.10.99` and `pyproject.toml` declares `>=0.10.13,<1.0` — so new installs get a build these detectors are actually tested against, and an install that already picked up 1.0.x is brought back to it. Anyone already on 0.10.x sees no change at all.
- **Fix: on macOS, MediaPipe detectors now bow out instead of taking the WebUI down with them.** As a second layer for setups that skip dependency installation, the extension checks before creating a MediaPipe detector and, **only on macOS with MediaPipe 1.0.1 or newer**, declines to use it: it prints a one-line console message telling you how to fix it (`pip install "mediapipe<1.0"`) and the MediaPipe detectors report "nothing detected" for that run. This second layer is macOS-only: on Windows and Linux the detector code is unchanged — the same call was verified to run normally on Windows with MediaPipe 1.0.1 on Python 3.13 — and every non-MediaPipe detector is unchanged on every platform. (The version cap above, by contrast, applies everywhere: an install that had picked up 1.0.x is brought back to 0.10.x on any OS.)
- **Fix: the MediaPipe tests no longer pass silently when MediaPipe is broken.** The test for these detectors only checked its results *if* MediaPipe returned something, so a completely non-working MediaPipe still produced a green test run — which is how a broken build could go unnoticed. It now reports a **skip** in that case instead of a pass, so the test summary says out loud that MediaPipe didn't run.

## v26.3.0+plus.7.3 — 2026-07-20 (pick which detections to inpaint)

Confirmed working on Forge Neo by @koblue (issue #5). Off by default, backward-compatible.

- **Fix: detections stay aligned with their box when masks are dropped or merged.** Two long-standing alignment bugs, fixed together (they became more visible with the numbered detections above): (1) sorting detections (e.g. by area) reordered the boxes but not their **confidence** values, so the confidence shown in the verbose log could belong to a different box; (2) when mask preprocessing **dropped an emptied mask** (aggressive erosion), **merged** masks, or (in img2img inpaint) dropped a non-overlapping one, the per-box **dynamic-denoise / resolution-scale**, **Auto class-guard** and inline **`[CLASS=]`** prompt could then read the *wrong* detection's box or class. Both are now kept in lock-step. No effect unless you use those options; the common case is byte-identical.
- **New: "Inpaint only these detections" — keep just the ones you pick.** Each ADetailer tab's Detection section gained a small text field: type the detection numbers you want to inpaint (e.g. `1,3,5`, or a range like `1-3`); leave it blank to inpaint every detection as before. To see the numbers, run the **Detection preview** — each detected box is now labelled with its number (`#1`, `#2`, …). Use it as a two-step flow: preview to see the numbers, type the ones you want, then Generate — it also works in the **Run ADetailer on an image** tool, where the "pause" is simply the gap between previewing and running. The numbers follow the raw detector order, so they only line up when the detector, confidence and detection resolution are the same between the preview and the run (that's why it's most reliable in the two-step preview→run flow and in the standalone tool). Ignored in **Process classes sequentially** mode (there each class is re-detected on its own, so the preview numbers wouldn't map — that pass details every detection of each class). If the numbers you typed match nothing this pass (e.g. the run found fewer regions than the preview), the console says so instead of leaving you guessing. Note: a true mid-generation pause isn't possible in the WebUI, so this is the practical equivalent. Index-safe: one new per-tab field wired like every other, plus a pure runtime filter and preview numbering — no new event handlers.
...
- **Fix: the CLASSES list now shows immediately at startup for an already-selected multi-class detector.** If a tab restarted with a multi-class detector already selected but **no** class filter saved, the CLASSES dropdown came up empty — you had to switch detector and switch back to make the classes appear. It now seeds the detector's full class list on load whenever the saved detector is class-based (any multiclass YOLO or the mediapipe face-features model), so every class is selectable straight away, with your saved selection (if any) pre-applied. Reads the model's `.names.json`/`.json` sidecar (fast, cached), so startup stays light.
- **Fix: missing gap in the Inpaint prompts section.** The **Auto class-guard / Class-guard emphasis** row sat flush against the class-prompts textbox above it. Added a small top margin to that one row so it no longer touches the textbox (other sections are unchanged).
...
- **New: HD detection — per-tab detector resolution.** A new **"Detection resolution"** slider in each tab's Detection section runs the YOLO detector at a higher inference resolution (e.g. 1024 instead of the default 640), so **small or distant faces/parts are detected better**. Default **0 = keep the default (640)**; higher values find more but use more VRAM and time (capped at 1536 in the UI). Detected boxes are rescaled to the original image internally, so nothing downstream changes. Applies to the generation pass, to **Run ADetailer on an image**, and to the **single-tab Detection preview** (so the preview now reflects what your setting will detect); the **Combine all tabs** preview stays at the default 640 (it only carries each tab's five detector fields, not the resolution). Off by default and backward-compatible.
...
- **New: built-in guide inside the extension — an opt-in top-level tab.** Turn on **`Settings → ADetailer → Show the ADetailer Guide tab`** (off by default; needs a UI reload) and a **"📖 ADetailer Guide"** tab appears in the main tab bar (next to Settings) with a **table of contents** and one **collapsible section per topic** (Getting started, Choosing a detector, Detection, Mask preprocessing, Prompts, Inpainting, Per-pass overrides, Tools, Presets, Fixing common problems, Special tokens, What's new) — a complete plain-language reference for every option, no need to open GitHub. When the tab is on, a small **"📖 Guide" quick-link** appears in the accordion header (next to the version badge) to open it; a non-invasive pointer at the bottom of the panel explains how to enable it. Registered via the core `on_ui_tabs` callback (guarded — a failure just yields no tab) as its own `gr.Blocks`, so it's fully isolated from the panel wiring and index-safe; read-only English reference text.
- **New: MediaPipe "face features" detector — detail individual facial parts.** A new detector **`mediapipe_face_features`** treats the face as a **multi-class** model whose classes are **eyes, mouth, nose, eyebrows, face**. Because it's class-based, it reuses the existing **CLASSES filter** — pick **all** (leave empty), **some**, or **just one** — plus include/exclude (NOT) mode, **Process classes sequentially** (a separate pass + prompt per part), **per-class prompts**, and **Auto class-guard** (each part guards itself against the others), with landmark-precise masks (not rectangles). Runs on MediaPipe's current `tasks` API (see the MediaPipe fix below), with a fallback to the legacy `solutions` API on older installs, and a hard-coded nose fallback so nose still works even if neither exposes it; degrades to "nothing detected" on any failure. The existing `mediapipe_face_mesh_eyes_only` is unchanged. Index-safe: reuses the class dropdown, adds no new listeners.
- **New: batch a whole folder in "Run ADetailer on an image".** The same bottom-of-tab tool now takes an optional **folder path**: paste one and the Run button details **every image inside the folder** with this tab's settings — detector, checkpoint, prompt, LoRAs, text encoder, VAE — in one click, always saving each result to the **`ADetailer-Inpaint`** outputs folder. A folder path takes priority over a single dropped image; a preview of the first results is shown in the gallery (all are written to disk). Fully guarded per file, so one unreadable image never aborts the batch. Index-safe: reuses the existing Run button (adds no event) and the folder box has no listener.
- **New: batch option to save each result back in the source folder.** A **"Save results in the source folder instead"** checkbox (off by default) makes the folder-batch write each detailed image **next to its source file** as `name-ad` (a new file), instead of the `ADetailer-Inpaint` folder — handy for touching up a folder of photos in place. **Your originals are never overwritten:** if the target name is already taken it uses `name-ad-1`, `name-ad-2`, and so on, and files already ending in `-ad` are skipped so a re-run doesn't reprocess earlier results. JPEGs are written at high quality (95). Index-safe: the checkbox is a listener-free input to the existing Run button (no new event, no new saved field).
- **New: inline `[CLASS=name] … [/CLASS]` per-region prompts.** Write class-specific text right inside a tab's **Prompt** / **Negative prompt** — e.g. `portrait [CLASS=hand] five fingers [/CLASS]`. Each detected region keeps only the blocks matching **its own detected class** and drops the rest; text outside any block applies to every region. Unlike the `ad_class_prompts` field (which needs sequential mode), this works in **normal and sequential** modes; the tag can target several classes at once (`[CLASS=face,eyes]`), and `[CLASS=hand] [SKIP] [/CLASS]` skips only that class. Pure runtime prompt rewrite applied per mask — **no new field, no new listener** (index-safe) — and a prompt with no `[CLASS=` tag is byte-identical, so it's inert unless you use it.
- **The single-tab Detection preview now honours the per-tab "Detection resolution".** The fast preview previously always ran at the detector default (640); it now uses this tab's HD resolution so the preview reflects what the real pass will detect. Combine-all-tabs stays at 640. No-op when the resolution is left at 0.
- **New: "active tabs" counter pill in the accordion header.** A small green pill (e.g. `2x Tabs`) sits **right after the "ADetailer" title** (hugging it, like ControlNet Integrated's unit counter — the badge is placed inside the header's title span) and shows how many detector tabs are active — i.e. how many "Enable this tab" checkboxes are ticked — updating live as you toggle them; it stays visible when the accordion is collapsed and hides at 0. Pure client-side (`javascript/tab-count-pill.js` + CSS), so it adds no Gradio component or listener — index-safe. Guarded writes (only on change) keep the DOM observer from busy-looping.
- **Fix: large images no longer bounce off the "Run ADetailer on an image" / batch / detection-preview tools.** Dropping a big upscale (e.g. a 216 MP image) raised PIL's `DecompressionBombError` inside Gradio's Image component — before the detector ever ran — showing a puzzling "no detection". The extension now raises PIL's decompression-bomb ceiling to a generous finite cap at load (only ever raising it, never lowering it or overriding a host that already disabled the check), so large finished images load and get detailed. The per-region working resolution is still capped internally to avoid OOM.
- **Fix: the class dropdown now shows the model's FULL class list on startup.** After a restart, a tab that restored a class filter showed only the classes you had already selected — to add another class you first had to re-toggle the detector (which repopulated the list). The startup restore now reads the model's full class list via its `.names.json`/`.json` sidecar (fast, no model load; only for tabs that actually restored a filter), so every class is selectable straight away, with your saved selection kept. Reported during testing.
- **Extra ADetailer tabs now start disabled.** Only the **1st tab** is enabled by default; the 2nd/3rd/… tabs start with "Enable this tab" **off** (they have no detector selected yet anyway), so a fresh setup runs just one detailer pass and the header pill reads `1x Tab`. Your saved / last-used per-tab state still wins, so nothing changes for an existing setup until you reset.
- **Fix: photos loaded from disk now honour their EXIF orientation.** Camera/phone photos store their orientation as an EXIF tag rather than rotating the pixels. The **batch-folder** tool opens files itself (not through Gradio), so a rotated photo reached the detector sideways — producing "nothing detected" or garbage boxes. The batch loader now applies the EXIF orientation, and the single-image **Run / Detection preview** paths do too as a guard (a strict no-op where the host WebUI already rotated it, so it can never double-rotate). Fixes real-camera portraits silently failing detection.
- **Fix: MediaPipe detectors work again on recent builds (Python 3.13 / Forge Neo).** Recent MediaPipe releases (0.10.x on Python 3.13, which current Forge Neo ships) removed the old `solutions` API completely, so **every** MediaPipe detector — `mediapipe_face_short`, `mediapipe_face_full`, `mediapipe_face_mesh`, `mediapipe_face_mesh_eyes_only` and the new `mediapipe_face_features` — silently returned "nothing detected" on any image (human or non-human subjects alike; it was never a furry/anthro issue). They are all ported to MediaPipe's current **`tasks`** API (`FaceDetector` / `FaceLandmarker`), which downloads two small model files (~3.8 MB + ~0.2 MB) into the extension's `_mediapipe_models/` folder on first use — or you can drop them in by hand if the machine is offline (the console prints the exact filenames and URLs). A fallback to the old `solutions` API is kept for older MediaPipe/Python installs, so A1111 / Forge / Forge Neo / reForge all keep working. Fully guarded: any download or model-load failure degrades to "nothing detected", never a crash, and a corrupt/partial download is rejected and re-fetched instead of being cached.
...
**No startup error when another ADetailer-family extension is installed.** When this fork sits in `extensions/` alongside the original ADetailer (or another variant), both register the same `--ad-no-huggingface` launch option and the second one to load raised an `Error running preload()` traceback at startup (reported in #1). The registration is now guarded: if the option already exists, the existing one is reused. Running two ADetailer variants at once remains unsupported — keep only one and move the other out of `extensions/` as a backup. The README's Install section now states this explicitly.
...
Wrap-up of the 2026-05-18..2026-05-19 test session. All 28 functional tests confirmed hands-on by the repo owner; 4 non-trivial bug fixes landed mid-test.
...
User request: "add a button in the settings that resets the extension's
own settings". Added a red button at
