...
- **Merge and Merge and Invert gave their region the first detection's class.** A merged mask holding a face and a hand got "face" from the Auto class-guard and "hand" in its negative prompt, and a Merge and Invert background got "face" in its positive prompt. Inline `[CLASS=…]` blocks followed the same wrong class. A merged region that mixes classes is now treated as class-less (no guard, every inline block kept); a Merge and Invert region, the background, gets no guard and no class block, so `[CLASS=hand] [SKIP] [/CLASS]` no longer skips it. Two merged faces are still a face.
