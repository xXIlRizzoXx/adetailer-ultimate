...
- **A prompt made only of `[CLASS=…]` blocks inpainted every other region with an empty prompt.** A blank ADetailer prompt means the main prompt, but `[CLASS=hand] [SKIP] [/CLASS]` alone, the documented way to skip only the hands, left the face with an empty prompt (or only the append text and the main prompt's LoRAs, when set); a negative prompt made only of blocks did the same. The text outside the blocks is blank, so it now means the main prompt, as a blank prompt does: the other regions get the main prompt, and a matching block is added to it (`[CLASS=hand] five fingers [/CLASS]` gives the hands the main prompt plus "five fingers" instead of "five fingers" alone). Earlier releases had the same problem.
- **On Forge and Forge Neo, the detailer pass ignored the Distilled CFG Scale / Shift** and always used the host's default, 3.5, while the main image used your value (Forge Neo's slider starts at 3.0, and its presets set other values for models such as Z-Image, Lumina or Wan). The Flux guidance or sampling shift of every detailed region therefore differed from the rest of the picture. The detailer pass now uses the main image's value. AUTOMATIC1111 has no such setting and is not affected. Earlier releases had the same problem.
...
- Some code comments in `style.css` were not in English; they are now in English.
